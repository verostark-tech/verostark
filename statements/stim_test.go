...
const syntheticCSV = `Work ID,Title,Source,Right Type,Gross,ISRC,Controlled by Publisher (%),Interested Party,Role,Manuscript Share (%),Amount before fee,Fee (%),Fee Amount,Net Amount
BM001,Sommarnatt,Internet,M,48.90,SE-K40-25-00001,1.0,Lindqvist Erik [00234567890],Composer&Lyricist,1.0,48.90,0.03,1.47,15.81
BM002,Langtan,Internet,M,44.47,SE-K40-25-00002,0.5,Holm Marcus [00456789012],Composer&Lyricist,1.0,44.47,0.03,1.33,7.19
BM003,Vintervag,Internet,M,56.77,SE-K40-25-00003,1.0,Strand Anna [00567890123],Composer&Lyricist,0.5,56.77,0.03,1.70,18.35
BM003,Vintervag,Internet,M,56.77,SE-K40-25-00003,1.0,Bjork Sara [00345678901],Lyricist,0.5,56.77,0.03,1.70,18.35
BM004,Drommar,Internet,M,84.02,SE-K40-25-00004,1.0,Lindqvist Erik [00234567890],Composer&Lyricist,1.0,84.02,0.03,2.52,81.50
BM005,Frihet,Internet,M,21.47,SE-K40-25-00005,1.0,Bjork Sara [00345678901],Composer&Lyricist,1.0,21.47,0.03,0.64,6.94
`
...
func TestParseSTIM_GrossAmountsPopulated(t *testing.T) {
	...
	wants := []float64{48.90, 44.47, 56.77, 56.77, 84.02, 21.47}
	for i, l := range lines {
...
func TestParseSTIM_NetAmounts(t *testing.T) {
	...
	wants := []float64{15.81, 7.19, 18.35, 18.35, 81.50, 6.94}
	for i, l := range lines {
...
func TestParseSTIM_WorkRefs(t *testing.T) {
	...
	wants := []string{"BM001", "BM002", "BM003", "BM003", "BM004", "BM005"}
	for i, l := range lines {
